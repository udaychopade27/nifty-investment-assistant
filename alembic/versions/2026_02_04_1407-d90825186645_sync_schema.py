...
def upgrade() -> None:
    ...
    crash_severity_enum = postgresql.ENUM('MILD', 'HIGH', 'EXTREME', name='crashseverityenum')
    crash_severity_enum.create(op.get_bind(), checkfirst=True)
    decision_type_enum = postgresql.ENUM('NONE', 'SMALL', 'MEDIUM', 'FULL', name='decisiontypeenum')
    decision_type_enum.create(op.get_bind(), checkfirst=True)
    etf_status_enum = postgresql.ENUM('PLANNED', 'SKIPPED', 'EXECUTED', 'REJECTED', name='etfstatusenum')
    etf_status_enum.create(op.get_bind(), checkfirst=True)
    ...
    op.alter_column('daily_decision', 'decision_type',
               ...
               postgresql_using='decision_type::decisiontypeenum')
    ...
    op.alter_column('etf_decision', 'status',
               ...
               existing_nullable=False,
               postgresql_using='status::etfstatusenum')
...
def downgrade() -> None:
    ...
    crash_severity_enum = postgresql.ENUM('MILD', 'HIGH', 'EXTREME', name='crashseverityenum')
    crash_severity_enum.drop(op.get_bind(), checkfirst=True)
    decision_type_enum = postgresql.ENUM('NONE', 'SMALL', 'MEDIUM', 'FULL', name='decisiontypeenum')
    decision_type_enum.drop(op.get_bind(), checkfirst=True)
    etf_status_enum = postgresql.ENUM('PLANNED', 'SKIPPED', 'EXECUTED', 'REJECTED', name='etfstatusenum')
    etf_status_enum.drop(op.get_bind(), checkfirst=True)
